...
%>
        ...
        <div class="ds44-form__container hidden">
            <div class="ds44-posRel">
                ...
                <input class="ds44-input-value" type="hidden" value=""><input class="ds44-input-metadata" type="hidden" value=""><input type="text" id="adresse-<%= idFormElement %>" name="adresse" class="ds44-inpStd" role="combobox" aria-autocomplete="list" autocomplete="off" aria-expanded="false" title='<%= JcmsUtil.glp(userLang,"jcmsplugin.socle.menu.pdcv.ajouterAdresse") %> - <%= JcmsUtil.glp(userLang,"jcmsplugin.socle.obligatoire") %>' data-url="https://api-adresse.data.gouv.fr/search/?limit=5&citycode=$parentValue" data-mode="select-only" required="" placeholder='<%= JcmsUtil.glp(userLang,"jcmsplugin.socle.menu.pdcv.votreAdresse") %>' aria-owns="owned_listbox_idpuhzgtecd3n"><button class="ds44-reset" type="button" style="display: none;"><i class="icon icon-cross icon--sizeL" aria-hidden="true"></i><span class="visually-hidden"><%= JcmsUtil.glp(userLang,"jcmsplugin.socle.facette.effacer-contenu-champ", JcmsUtil.glp(userLang,"jcmsplugin.socle.menu.pdcv.ajouterAdresse")) %></span></button>
